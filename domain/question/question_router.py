...
@router.get("/list", response_model=question_schema.QuestionList)   # 리턴타입 = Question_schema의 QuestionList
...
def question_update(_question_update: question_schema.QuestionUpdate,
    ...
    if current_user.id != db_question.user.id:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,        # 본인이작성한 게시글만 수정 가능
                            detail="수정 권한이 없습니다.")
...
# /api/question/list 라는 URL 요청이 발생하면 /api/question 이라는 prefix가 등록된
#  question_router.py 파일의 /list로 등록된 함수 question_list가 실행되는 것이다.