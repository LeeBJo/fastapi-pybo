...
from testModel import Question, User, Answer
from sqlalchemy.orm import Session


def get_question_list(db: Session, skip: int = 0, limit: int = 10, keyword: str = ''): #skip:조회데이터의 시작위치, limit:시작위치로부터 가져올 데이터 건수
    question_list = db.query(Question)
    if keyword:
        search = '%%{}%%'.format(keyword)
        sub_query = db.query(Answer.question_id, Answer.content, User.username) \
            .outerjoin(User, Answer.user_id == User.id).subquery()      # c는 서브쿼리의 조회 항목
